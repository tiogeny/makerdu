...
class AiQualityControlService
{
    ...
    public function analyzeFile(string $filePath, string $originalFilename, ?array $rules = [], ?string $imageBase64 = null): array
    {
        ...
        if ($metrics['x_mm'] > $maxX) {
            $violations[] = "El ancho X ({$metrics['x_mm']} mm) supera el lÃ­mite mÃ¡ximo ({$maxX} mm).";
        }
        if ($metrics['y_mm'] > $maxY) {
            $violations[] = "El largo Y ({$metrics['y_mm']} mm) supera el lÃ­mite mÃ¡ximo ({$maxY} mm).";
        }
        if ($metrics['z_mm'] > $maxZ) {
            $violations[] = "La altura Z ({$metrics['z_mm']} mm) supera el lÃ­mite mÃ¡ximo ({$maxZ} mm).";
        }

        $isValid = count($violations) === 0;

        // 3. Generar feedback con VisiÃ³n Artificial (Gemini 3.6 Flash / Fallback estructurado)
    ...
    private function generateAiFeedback(string $fileName, array $metrics, array $violations, bool $isValid, array $rules = [], ?string $imageBase64 = null): array
    {
        ...
        // Prioridad: Modelos Lite (500 peticiones/dÃ­a gratuitas) antes de modelos estÃ¡ndar (20 peticiones/dÃ­a)
        ...
        if ($apiKey) {
            if ($isImageDrawing) {
                $promptText = "Eres el Mentor de FabricaciÃ³n Digital y Control de Calidad de Makerdu. "
                    . "Analiza la imagen adjunta correspondiente a un boceto o silueta 2D para un Art Toy 2.5D autoportante ('{$fileName}'). "
                    . "EvalÃºa con visiÃ³n pedagÃ³gica y tÃ©cnica: "
                    . "1. Identifica quÃ© personaje o figura es (ej. dinosaurio, criatura, robot, animal). "
                    . "2. Â¿La silueta exterior estÃ¡ completamente cerrada o tiene aberturas? "
                    . "3. Â¿La base inferior es amplia y estable para que el juguete se pare de pie solo en el escritorio? "
                    . "4. Â¿Tiene espacios negativos bien definidos (ojos, boca, detalles)? "
                    . "5. CRÃTICO (Regla de Estarcido / Stencil): Â¿Tiene 'islas flotantes' o elementos desprendidos (ej. una pupila negra flotando dentro de un ojo blanco sin puentes que la unan al cuerpo)? Si detectas piezas flotantes desconectadas, ADVIERTE en pedagogical_tip que se caerÃ¡n al cortar o extruir a menos que tengan un puente de uniÃ³n o una base sÃ³lida. "
                    . "Devuelve ÃšNICAMENTE un objeto JSON vÃ¡lido con este formato exacto: "
                    . json_encode([
                        'headline' => 'Criatura cÃ­clope con base amplia autoportante',
                        'strengths' => [
                        ...
                        'pedagogical_tip' => 'Consejo tÃ©cnico sobre uniones de patas y advertencia si hay islas flotantes sueltas (como pupilas sin puentes)',
                        'verdict_title' => 'Â¡SILUETA APROBADA!',
                        'text_summary' => 'Resumen amigable de 2 oraciones para el estudiante creador'
                    ]);
            } else {
                $promptText = "Eres el Ingeniero Jefe de FabricaciÃ³n Digital y Control de Calidad de Makerdu. "
                    . "Analiza la imagen adjunta del modelo 3D '{$fileName}' ({$metrics['x_mm']}x{$metrics['y_mm']}x{$metrics['z_mm']} mm, {$metrics['material_grams']}g PLA, " . ($metrics['triangles'] ?? 0) . " triÃ¡ngulos). "
                    . "LÃ­mites del reto: {$maxX}x{$maxY}x{$maxZ} mm. "
                    . "Estado: " . ($isValid ? "APROBADO" : "RECHAZADO: " . implode('; ', $violations)) . ". "
                    . "Devuelve ÃšNICAMENTE un objeto JSON vÃ¡lido con este formato exacto: "
                    . json_encode([
                        'headline' => 'Tipo de objeto y silueta detectada en 1 frase corta',
                        'strengths' => [
                            'Punto fuerte 1 sobre la geometrÃ­a o adherencia',
                        ...
                        'pedagogical_tip' => 'Consejo tÃ©cnico breve para el estudiante',
