...
/**
 * El QR siempre usa `qrUrl` (inyectado por el controlador con getSchemeAndHttpHost()
 * que en producción es https://makerdu.com). Si no se recibe, fallback al window.location.
 */
const resolvedQrUrl = props.qrUrl
    || (typeof window !== 'undefined'
        ? `${window.location.origin}/family/${props.classroom?.access_code}/squad/${props.squad?.id}`
        : `https://makerdu.com/family/${props.classroom?.access_code}/squad/${props.squad?.id}`);
...
<template>
    <Head :title="`${t('passport.title')} - ${squad.name}`" />
        ...
        <main class="max-w-5xl mx-auto p-4 sm:p-8 space-y-8 print:p-0 print:space-y-4">
            ...
            <div class="rounded-3xl bg-gradient-to-b from-slate-900 via-slate-900 to-cyan-950/40 border-2 border-amber-500/50 p-8 sm:p-12 shadow-2xl relative overflow-hidden print:border-2 print:border-black print:bg-white print:p-6">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between gap-6 border-b border-slate-800 print:border-black pb-8">
                    ...
                    <div class="text-center sm:text-right flex items-center gap-4">
                        <div class="text-right">
                            <p class="text-xs font-bold text-slate-400 print:text-black">{{ t('passport.xp_total') }}</p>
                            <p class="text-2xl font-black font-mono text-purple-400 print:text-black">
                                {{ squad.members.reduce((a, m) => a + (m.xp_points || 0), 0) }} XP
                            </p>
                        </div>
                        <!-- QR DINÁMICO: usa qrUrl inyectado por el servidor (siempre dominio de producción) -->
                        <div class="w-20 h-20 rounded-2xl bg-white p-1.5 flex items-center justify-center shadow-md border border-slate-700">
                            <img
                                :src="`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(resolvedQrUrl)}`"
                                alt="QR Portal Familiar"
                ...
                <div class="py-6 border-b border-slate-800 print:border-black">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400 print:text-black mb-3">{{ t('passport.team_title') }}:</h3>
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        ...
                        >
                            ...
                            <p class="text-[10px] text-cyan-400 print:text-black font-semibold mt-0.5">{{ m.pivot?.current_role || m.role }}</p>
                ...
                <div class="py-6 space-y-3">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-emerald-400 print:text-black flex items-center gap-1.5">
                        ...
                        <span>{{ t('passport.competencies_title') }}:</span>
                    </h3>
                ...
                <div class="pt-8 flex items-center justify-between text-xs text-slate-400 print:text-black border-t border-slate-800 print:border-black">
                    <div>
                        ...
                        <p class="text-[10px]">{{ t('passport.teacher_label') }}</p>
                    </div>
                    <div class="text-right">
                        <p class="font-bold text-amber-400 print:text-black">{{ t('passport.version_label') }}</p>
                        <p class="text-[10px]">{{ t('passport.cert_label') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4 print:page-break-before">
                <div class="flex items-center gap-2">
                    ...
                    <h2 class="text-lg font-black text-white">{{ t('passport.portfolio_title') }}</h2>
                </div>

                <div v-if="bitacoras && bitacoras.length > 0" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div
                    ...
                    >
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-bold text-cyan-300">Nivel {{ b.level?.level_number }}: {{ b.level?.title_json?.es || b.level?.title_json?.en || 'Reto Maker' }}</span>
                            <span class="text-[10px] font-bold text-emerald-400 bg-emerald-950/60 px-2 py-0.5 rounded-lg border border-emerald-600/40">
                                {{ t('passport.approved_badge') }}
                            </span>
                        </div>
                        ...
                        <div v-if="b.ai_feedback" class="p-3 rounded-xl bg-slate-950 border border-slate-800 text-[11px] text-cyan-300">
                            🤖 <strong>{{ t('passport.ai_diagnosis') }}:</strong> {{ b.ai_feedback }}
                        </div>
                    </div>
                </div>

                <div v-else class="p-8 rounded-3xl bg-slate-900/60 border border-slate-800 text-center">
                    <p class="text-sm text-slate-400">Aún no hay evidencias en la bitácora. ¡Completa misiones para llenar tu portafolio!</p>
                </div>
            </div>
